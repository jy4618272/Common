...
{
    /// <summary>
    /// Displays the values of a single record from a data source in a table, where each data row represents a field of the record. 
    ...
    public class MagicForm : DetailsView, IThemeable
    {
        ...
        {
            if (cell.Attributes["SpannedCell"] == "text")
            {
                ...
                if ((cell.Text.Trim().Length > 0) || (cell.Controls.Count > 0))
                {
                    ...
                    if (this.Theme != MasterPageTheme.Modern)
                    {
                        if (!(field is AutoGeneratedButtonsField))
                        {
