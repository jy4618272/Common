...
"
        ...
        private static Control CreateRadioButton(Setting setting, bool value, bool selectedValue, bool allowDisableState, int visibleChildSettingsCount)
        {
            ...
            try
            {
                ...
                if (allowDisableState && (visibleChildSettingsCount > 0))
                    str = string.Concat("ChangeDisabledState('", str, "', ", (value ? "false" : "true"), ");");
                else
        ...
        private void LoadResources()
        {
            UpdateButton.Text = MagicForm.GetUpdateButtonText(DetailsViewMode.Edit, string.Empty, InsertButtonCaptionType.Create, UpdateButtonCaptionType.Save
                , ((FrameworkConfiguration.Current.WebApplication.MasterPage.Theme == Pages.MasterPageTheme.Modern) ? CloseButtonVisibilityMode.None : CloseButtonVisibilityMode.Edit));
        ...
        private void CreateSettingControl(Setting setting, Control container, int visibleChildSettingsCount)
        {
            ...
            switch (setting.SettingType)
            {
                case SettingType.OnOffSwitch:
                    ...
                    container.Controls.Add(CreateRadioButton(setting, true, value, false, visibleChildSettingsCount));
                    container.Controls.Add(CreateRadioButton(setting, false, value, false, visibleChildSettingsCount));
                    break;
                case SettingType.CheckBox:
                    container.Controls.Add(CreateCheckBox(setting, false, this.DiagnoseConflictingSettings, visibleChildSettingsCount));
                    break;
        ...
        private void Repeater_ItemDataBound(RepeaterItemEventArgs e, string controlHolderId)
        {
            ...
            {
                CreateSettingControl(setting, controlHoder1, childSettings.Count);
                leftMargin = true;
            }
            ...
            if (setting.Action != null)
            {
                if (!string.IsNullOrEmpty(setting.Action.LearnMoreUrl))
                {
                    using (HyperLink learnMoreLink = new HyperLink())
                    {
                        ...
                        controlHoder1.Controls.Add(learnMoreLink);
                        leftMargin = true;
                    }
                }
            }
            ...
            if ((!setting.ParentSettingId.HasValue) && (!string.IsNullOrEmpty(setting.PaidUpgradeUrl)))
            {
                using (HyperLink paidUpgradeLink = new HyperLink())
                {
                    ...
                    controlHoder1.Controls.Add(paidUpgradeLink);
                }
            }
        ...
        protected void UpdateButton_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(CancelLink.NavigateUrl) || (FrameworkConfiguration.Current.WebApplication.MasterPage.Theme == Pages.MasterPageTheme.Modern))
            {
                this.List_DataBind();
