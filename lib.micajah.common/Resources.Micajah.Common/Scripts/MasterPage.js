...
function Mp_SafeCharsConvert(val) {
    val = val.replace(new RegExp('&amp;', 'g'), '-amp;');
    val = val.replace(new RegExp('&lt;', 'g'), '-lt;');
    val = val.replace(new RegExp('&gt;', 'g'), '-gt;');
...
function Mp_EncodeTextBoxes() {
    ...
    for (i = 0; i < inputs.length; i++) {
        ...
        if (input.type == "text") {
            input.disabled = true;
            input.value = Mp_SafeCharsConvert(input.value);
...
if (typeof ($) !== "undefined") {
    if (typeof ($.fancybox) == "function") {
        var originalInit = $.fancybox.init;
