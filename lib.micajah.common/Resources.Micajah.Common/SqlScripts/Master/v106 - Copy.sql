...
IF @@TRANCOUNT = 1
   ...
   [HowYouHearAboutUs] [nvarchar] (255) NOT NULL CONSTRAINT [DF_Mc_Organization_HowYouHearAboutUs] DEFAULT ('')
) ON [PRIMARY]
...
IF @@TRANCOUNT = 1
   ALTER TABLE [dbo].[Mc_Organization] ADD CONSTRAINT [PK_Mc_Organization] PRIMARY KEY CLUSTERED ([OrganizationId]) WITH (DATA_COMPRESSION = NONE) ON [PRIMARY]

IF @@ERROR <> 0
   IF @@TRANCOUNT = 1 ROLLBACK TRANSACTION

IF @@TRANCOUNT = 1
   CREATE INDEX [IX_Mc_Organization_PseudoId] ON [dbo].[Mc_Organization] ([PseudoId]) WITH (DATA_COMPRESSION = NONE) ON [PRIMARY]
